	...
	public sealed class Player : NetworkBehaviour
	{
		...
		private void ShowFireEffects()
		{
			...
			if (_visibleFireCount < _fireCount)
			{
				...
				if (weaponAnimators.Length > 0)
				{
					weaponAnimators[currentWeapon].SetTrigger("Shoot");

					currentWeapon++;

					if (currentWeapon >= weaponAnimators.Length)
					{
						currentWeapon = 0;
					}
                }
		...
		{
			_animIDSpeedX = Animator.StringToHash("Horizontal");
			_animIDSpeedZ = Animator.StringToHash("Vertical");
