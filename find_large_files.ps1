...
$total = $trackedFiles.Count
$i = 0
$files = foreach ($path in $trackedFiles) {
    $i++
    Write-Progress -Activity "Scanning tracked files..." -Status "$i / $total" -PercentComplete (($i / $total) * 100)

    # Convert to absolute path, handle spaces/special chars safely
